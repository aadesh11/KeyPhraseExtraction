...
def main():
    text = """
                 ...
                 is the process that assigns labels to words or phrases in a sentence that indicates their semantic role in the 
                 sentence, such as that of an agent, goal, or result.It serves to find the meaning of the sentence. To do this, 
                 it detects the arguments associated with the predicate or verb of a sentence and how they are classified into their 
                 specific roles. A common example is the sentence "Mary sold the book to John." The agent is "Mary," the predicate is 
                 "sold" (or rather, "to sell,") the theme is "the book," and the recipient is "John." Another example is how "the 
    ...
    run_algo = RunAlgo(top_k=TOP_K, text_embedding=text_embeddings, candidate_embeddings=candidates_embeddings,
                       candidates=candidate_keyphrase)
    keywords_without_mmr = run_algo.select_topn_candidates()
    keywords_wth_mmr, relavance, aliases_keywords = run_algo.mmr(beta=BETA, alias_threshold=ALIAS_THRESHOLD)
