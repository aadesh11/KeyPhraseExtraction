...
class ExtractCandidates:
    def __init__(self, ngram_range, texts):
        ...
        self.is_stop_word = (
            ...
            or token.lower_ in self.stop_words
            or token.lemma_ in self.stop_words
        )
    ...
    def create_candidate_with_noun(
        self,
    ):
    ...
    def get_candidates(self, only_nouns=False, using_count_vector=True):
        if using_count_vector:
            ...
            n_gram_candidates = (
                self.extract_all_candidates_from_text_using_count_vector()
            )
        else:
        ...
        filtered_candidates = list(
            filter(lambda candidate: candidate in all_nouns_and_np, n_gram_candidates)
        )
